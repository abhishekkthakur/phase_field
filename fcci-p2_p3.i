...
[Materials]
  [./fl]
    ...
    f_name = fl
    args = 'xnd xas'
    # Gibbs energy expression for phase 2
    function = '-12.572 + 7*((xnd-0.5)*(xnd-0.5) + (xas-0.5)*(xas-0.5))
                + 0.5*8.314*600*(2*(1-xnd-xas)*log(2*(1-xnd-xas)) + (1-2*(1-xnd-xas))*log(1-2*(1-xnd-xas)))
  ...
  [./fs]
    ...
    f_name = fs
    args = 'xnd xas'
    # Gibbs energy expression for phase 3
    function = '(1-xnd-xas)*1000 + xnd*5000 + xas*1000
                + 0.42857*8.314*600*((2.33)*xnd*log((2.33)*xnd) + (1-(2.33)*xnd)*log(1-(2.33)*xnd))'
  [../]
